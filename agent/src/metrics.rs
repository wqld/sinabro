...
use crate::{controller::Sinabro, Error};
...
impl Default for Metrics {
    fn default() -> Self {
        let reconcile_duration = HistogramVec::new(
            histogram_opts!(
                "sinabro_controller_reconcile_duration_seconds",
        ...
        let failures = IntCounterVec::new(
            opts!(
                "sinabro_controller_reconciliation_errors_total",
        ...
        let reconciliations = IntCounter::new(
            "sinabro_controller_reconciliations_total",
            "reconciliations",
        )
        .unwrap();
...
impl Metrics {
    ...
    pub fn reconcile_failure(&self, sinabro: &Sinabro, e: &Error) {
        self.failures
            .with_label_values(&[sinabro.name_any().as_ref(), e.metric_label().as_ref()])
            .inc()
    }
